...
// const removePop = ()=>{
//     const popE = document.querySelector(".popEdit");
//     popE.classList.remove("eff");
// }

// const removePopDelete = ()=>{
//     const popE = document.querySelector(".popDelete");
//     popE.classList.remove("eff");
// }
...
const deleteElement = (post, user) => {
    if(post.username === user.username){
        ...
        axios.delete(`http://localhost:5000/api/post/${post._id}`).then(() => {
            window.location.reload();
        })
    }
...
export default function Post({post}) {
    ...
    const Inc = (post)=>{
        ...
        try{
            axios.put(`http://localhost:5000/api/post/like/${post._id}`, {username: user.username})
            window.location.reload()
        }catch(err){
            console.log(err)
        }
    ...
    const Dec = (post)=>{
        try{
            axios.put(`http://localhost:5000/api/post/dislike/${post._id}`, {username: user.username})
            window.location.reload()
        }catch(err){
            console.log(err)
        }
    ...
    return (
        <>
            <div className="popEdit">
                Post Edited
                {/* <ImCross width="40px" className="cross" onClick={removePop}/> */}
            </div>
            <div className="popDelete">
                Post Deleted
                {/* <ImCross width="40px" className="cross" onClick={removePopDelete}/> */}
            </div>
            <div className="chatArea">
                ...
                <textarea id={post._id} className="mes" cols="33" rows="5" readOnly ref={myText}>{post.desc}</textarea>
                <hr className="hrLine"/>
